...
"""
Input-space adversarial training with an ICNN adversary.

This script mirrors the overall structure of ``pretrained_LAT_icnn.py`` but the
ICNN now operates directly on normalized CIFAR-10 inputs (no intermediate latent
encoder). The pretrained network is fine-tuned on these input-space adversarial
examples generated by the implicit optimal transport map
:math:`T(x) = \\nabla_x \\phi_\\omega(x)`.

Training alternates between:

* **Adversary step (gradient ascent on ``ω``)**: Maximize the robust loss
  :math:`\\mathbb{E}[\\ell(f_{\\theta}(T(x)), y) - \\lambda \\|x - T(x)\\|^2]`.
* **Model step (gradient descent on ``θ``)**: Update the full classifier on the
  adversarial inputs, treating the ICNN parameters as constants (Danskin-style
  outer gradient).

Most utilities (data loading, evaluation, checkpoint helpers) are reused from
...
def adversarial_pushforward(
    ...
    if detach_for_model:
        ...
        z_push = _clamped_normalized_copy(z_push).detach()
    ...
    with torch.enable_grad():
        ...
        z_adv = _clamped_normalized_copy(z_adv)
    delta = z_adv - z_leaf
    return z_adv, delta
...
def _clamped_normalized_copy(x: torch.Tensor) -> torch.Tensor:
    """Return a clamped copy of normalized CIFAR inputs within valid pixel bounds."""
    mean = torch.as_tensor(CIFAR10_MEAN, device=x.device, dtype=x.dtype).view(1, 3, 1, 1)
    std = torch.as_tensor(CIFAR10_STD, device=x.device, dtype=x.dtype).view(1, 3, 1, 1)
    lower = (0.0 - mean) / std
    upper = (1.0 - mean) / std
...
def visualize_adversarial_images(
    ...
    epoch_label = "final" if epoch_index is None else f"epoch-{epoch_index:03d}"
    phase_label = (phase_name or "final").lower()
    out_dir = Path(args.adv_image_dir) / run_id / epoch_label / phase_label
    ...
    plot_params = {
        ...
        "steps": args.adv_image_steps,
        "lr": args.adv_image_lr,
        "lambda": f"{float(args.penalty_lambda):.4g}",
        "icnn": "-".join(map(str, args.icnn_hidden)),
    }
    ...
    for idx in range(num_samples):
        sample_summary = {
            "index": int(idx),
            "label": int(y_batch[idx]),
            "clean_pred": int(preds_clean[idx]),
            "clean_conf": float(conf_clean[idx]),
            "transport_pred": int(preds_adv_latent[idx]),
            "transport_conf": float(conf_adv_latent[idx]),
            "recon_pred": int(preds_recon[idx]),
            "recon_conf": float(conf_recon[idx]),
            "delta_z_l2": float(delta_norm[idx]),
            "latent_mse": float(latent_mse[idx]),
            "pixel_l2": float(pixel_l2[idx]),
            "pixel_linf": float(pixel_linf[idx]),
        }
    ...
    for key in hyper_keys:
        if hasattr(args, key):
            ...
            if isinstance(value, (list, tuple)):
                hyper_params[key] = list(value)
            else:
                hyper_params[key] = value
    metadata = {
        "run_id": run_id,
        "epoch": None if epoch_index is None else int(epoch_index),
        "phase": phase_label,
        "split": split_name,
    ...
    meta_path.write_text(json.dumps(metadata, indent=2))
    print(f"Saved adversarial image metadata to {meta_path}")
...
def evaluate_under_icnn(
    ...
    for x, y in progress:
        ...
        z_adv = icnn.gradient(z_det, create_graph=False)
        z_adv = _clamped_normalized_copy(z_adv).detach()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Adversarial fine-tuning with an ICNN adversary operating in input space."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Run classifier finetuning epochs using fixed ICNN adversarial inputs.",
    )
    ...
    parser.add_argument(
        ...
        default="input",
        choices=["input"],
        help="Feature cut for ICNN inputs (input-space script always uses raw inputs).",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If set, save adversarial image panels at the end of every epoch.",
    )
    parser.add_argument(
...
def main():
    ...
    phi = nn.Identity().to(device)
    head = base_pretrained

    was_training = phi.training
    phi.eval()
    with torch.no_grad():
        example_batch = next(iter(trainloader))[0][:1].to(device)
        input_example = phi(example_batch)
        latent_dim = input_example[0].numel()
        latent_shape = input_example.shape[1:]
    phi.train(was_training)
    print(f"Input shape (normalized space): {latent_shape} (dim={latent_dim})")
    ...
    for epoch, phase in enumerate(schedule, start=1):
        ...
        if not minimax_sanity_done and phase == "icnn":
            print("[Sanity] ICNN pretraining complete. Evaluating clean model before minimax stage...")
        ...
        if args.visualize_adversarial_images and args.adv_image_every_epoch:
