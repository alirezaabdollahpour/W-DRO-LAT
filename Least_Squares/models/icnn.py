"""Input-convex potential (Amos et al., 2017) with principled non-negative init.

Used as the WDRO adversary potential in the ICNN baseline.
"""
...
def icnn_principled_moments(fan_in: int):
    """Hoedt & Klambauer (2023) LogNormal init for ICNN non-negative weights."""
    if fan_in <= 0:
...
class NonNegativeLinear(nn.Module):
    """Linear map with non-negative weights via exp (principled) or softplus (xavier)."""
...
class InputConvexPotential(nn.Module):
    """Scalar convex potential psi(z) parameterised as an ICNN + strong-convexity term."""
        ...
        for i, width in enumerate(hidden_sizes):
            ...
            if i == 0:
                ...
                self.h_linears.append(
                    NonNegativeLinear(
                        hidden_sizes[i - 1], width, bias=False, init_mode=nonneg_init,
                    )
                )

        self.hidden_output = NonNegativeLinear(
            hidden_sizes[-1], 1, bias=True, init_mode=nonneg_init,
        )
