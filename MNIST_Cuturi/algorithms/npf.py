...
def train_step_npf(
    ...
    cfg: TrainConfig,
    device: torch.device,
) -> Tuple[TrainState, NPFICNNState, Dict[str, torch.Tensor]]:
    model = state.model
    opt = state.opt
    x, y = batch
    x = x.to(device)
    ...
    def adv_obj_params(vec: torch.Tensor, create_graph: bool) -> torch.Tensor:
        ...
        adv_loss = adversary_loss(logits, y, cfg.use_margin_adv_npf)
...
def train_algorithm_npf(
    cfg: TrainConfig, device: torch.device
) -> Tuple[TrainState, NPFICNNState, Dict[str, Any]]:
    """NPF-style WDRO adversarial training (outer structure mirrors Algo2)."""
    ...
    input_dim = 28 * 28 * 1
    icnn_state = create_npf_icnn_state(cfg, input_dim, device)
    ...
    for epoch in range(cfg.num_epochs):
        ...
        if is_clean:
            ...
            for step, (x, y) in enumerate(train_loader):
                if cfg.max_steps_npf is not None and step >= cfg.max_steps_npf:
                    break
                n = x.size(0)
            ...
            if num_steps > 0:
                ...
                print(
                    f"[NPF] Epoch {epoch} (clean) loss={avg_loss:.4f} "
                    f"acc_clean={avg_acc_clean:.4f}"
                )
                logger.log(
                    algorithm="npf", phase="train_clean", epoch=epoch,
                ...
                print(
                    f"[NPF] Epoch {epoch}  val_clean={val_c:.4f}  "
                    f"score={score:.4f}"
                )
                if score > best_score:
                    best_score = score
            ...
            for step, (x, y) in enumerate(train_loader):
                if cfg.max_steps_npf is not None and step >= cfg.max_steps_npf:
                    break
                n = x.size(0)
                state, icnn_state, metrics = train_step_npf(
            ...
            if num_steps > 0:
                ...
                print(
                    f"[NPF] Epoch {epoch} (adv) adv_loss={avg_adv_loss:.4f}"
                ...
                logger.log(
                    algorithm="npf", phase="train_adv", epoch=epoch,
                ...
                if cfg.es_patience > 0 and patience_count >= cfg.es_patience:
                    print(
                        f"[NPF] Early stopping at epoch {epoch} "
    ...
    print("[NPF] Test:", test_metrics)
    logger.log(
        algorithm="npf", phase="test", epoch=cfg.num_epochs, step=None,
    ...
    with open(os.path.join("MNIST", "npf_icnn_results.json"), "w") as f:
