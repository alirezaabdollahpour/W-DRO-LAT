...
Architecture: per-layer diagonal+low-rank convex quadratic injections on top of
a learnable outer PSD base, with non-negative hidden-to-hidden weights and a
convex non-decreasing activation (ELU by default).
"""
...
class NPFNonNegativeDense(nn.Module):
    """Non-negative linear layer with Hoedt & Klambauer (we call it "principled weight initialization in our paper") negative bias init."""
...
class NPFInputConvexPotential(nn.Module):
    """NPF Eq. (5) ICNN potential: learnable PSD base + deep quadratic block. (I have to double check it with Marco)"""
